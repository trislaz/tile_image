...
for o, n in enumerate(ts.store_tile):
    var = ts.target_correspondance[o]
    tiles = ts.store_tile[n]
    tiles = np.vstack(tiles)
    np.save(os.path.join(out_encoded, var+'.npy'), tiles)
    attention = np.vstack(ts.store_attention[o])
    preclassif = np.vstack(ts.store_preclassif[o])
    scores = np.vstack(ts.store_score[o])
    np.save(os.path.join(out_encoded, var+'_attention.npy'), attention)
    np.save(os.path.join(out_encoded, var+'_preclassif.npy'), preclassif)
    np.save(os.path.join(out_encoded, var+'_scores.npy'), scores)
